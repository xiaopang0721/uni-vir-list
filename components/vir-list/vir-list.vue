<template>
	<view ref="scroller" scroll-y class="height-dynamic" @scroll="handleScroll">
		...
		<item class="height-dynamic__item" v-for="item in visibleData" ref="items" :data="item" :key="item.index"
		 :index="item.index" :style="`transform: translate(0, ${cachedScrollY[item.index] || item.index * ESTIMATED_HEIGHT}px)`"
		 @size-change="handleSizeChange">
		 <!-- <slot name="default" :data="item" :index="item.index"></slot> -->
		 </item>
	</view>
</template>

<script>
	import {http} from '../../api/services/http.js'
	...
	export default {
		...
		computed: {
			...
			VISIBLE_COUNT() {
				return Math.ceil(this.$refs.scroller.$el.offsetHeight / ESTIMATED_HEIGHT);
			}
		},
		mounted() {
		...
		methods: {
			...
			handleScroll() {
				...
				const delta = this.$refs.scroller.$el.scrollTop - this.lastScrollTop;
				this.lastScrollTop = this.$refs.scroller.$el.scrollTop;
			...
			async updateAnchorItem(delta) {
				...
				if (this.cachedScrollY[this.firstAttachedItem] <= -1) {
					...
					const actualScrollY = this.cachedHeight.slice(0, Math.max(0, this.anchorItem.index)).reduce((sum, h) => (sum += h),
						0);
					this.$refs.scroller.$el.scrollTop = actualScrollY + this.anchorItem.offset;
					this.lastScrollTop = this.$refs.scroller.$el.scrollTop;
			...
			async calItemScrollY() {
				...
				this.$set(this.cachedScrollY, this.anchorItem.index, this.$refs.scroller.$el.scrollTop - this.anchorItem.offset);
			...
			handleLoadMore() {
				const scrollEnd = this.$refs.scroller.$el.scrollTop + this.$refs.scroller.$el.offsetHeight + ESTIMATED_HEIGHT;
